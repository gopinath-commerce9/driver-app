...
import { DeliveriesPageService } from './../../services/deliveries-page.service';
...
export class DeliveriesPage implements OnInit {
  ...
  constructor(
    ...
    private service: DeliveriesPageService
  ) {
    console.log('DeliveriesPage constructor');
    this.appTitle = appConfigs.appMainTitle;
    this.pageTitle = 'Out-For-Delivery Orders';
  }

  ngOnInit() {
    console.log('DeliveriesPage ngOnInit');
    ...
    this.displayLoader().then((loader) => {
      ...
      this.activatedRoute.queryParams.subscribe((paramData) => {
        ...
        this.pageCount = 0;
        this.deliveriesList = [];
        this.service.getDeliveryList(this.pageCount, this.itemLimit).subscribe((result) => {
          if (Array.isArray(result) && (result.length > 0)) {
            result.forEach((delivery: any, notifyIndex: number) => {
              this.deliveriesList.push(delivery);
            });
          }
          loader.dismiss();
        }, (err) => {
  ...
  ionViewWillEnter() {
    console.log('DeliveriesPage ionViewWillEnter');
  }

  ionViewDidEnter() {
    console.log('DeliveriesPage ionViewDidEnter');
  }

  ionViewWillLeave() {
    console.log('DeliveriesPage ionViewWillLeave');
  }
  ...
  loadOlderData(event) {
    setTimeout(() => {
      ...
      this.pageCount++;
      this.updateDeliveryList(1);
      event.target.complete();
    }, 1000);
  }

  updateDeliveryList(direction: number = 0) {
    ...
    this.service.getDeliveryList(this.pageCount, this.itemLimit).subscribe((result) => {
        ...
        result.forEach((delivery: any, notifyIndex: number) => {
          ...
          if (currentOrderId !== '') {
            ...
            if (this.deliveriesList.length > 0) {
              checkArray = this.deliveriesList.filter(
            ...
            if (checkArray.length === 0) {
              ...
              if (insideDelivery.orderStatus !== currentOrderStatus) {
                this.deliveriesList.forEach((value, index)=>{
                  if(value.recordId === insideDelivery.recordId) {
                    this.deliveriesList.splice(index, 1);
                  }
                });
                tempArray.push(delivery);
              }
      ...
      if (tempArray.length > 0) {
        if (directionClean === 0) {
          this.deliveriesList = [...tempArray, ...this.deliveriesList];
          this.changeDetect.detectChanges();
        } else if (directionClean === 1) {
          this.deliveriesList = [...this.deliveriesList, ...tempArray];
          this.changeDetect.detectChanges();
        }
      }
    });
